...
internal fun <T> newMutableList(): MutableList<T> = MutableOnWriteList(emptyList())
...
internal fun <T> copyOf(list: List<T>): MutableList<T> {
...
internal fun <K, V> copyOf(map: Map<K, V>): MutableMap<K, V> = LinkedHashMap(map)

internal fun <T> immutableCopyOf(name: String, list: List<T>): List<T> {
  var list = list
...
internal fun <K, V> immutableCopyOf(name: String, map: Map<K?, V?>): Map<K, V> {
  if (map.isEmpty()) {
    return emptyMap()
  }
...
@JvmName("-redactElements") // Hide from Java
internal fun <K, V> Map<K, V>.redactElements(adapter: ProtoAdapter<V>): Map<K, V> {
  return mapValues { (_, value) -> adapter.redact(value) }
}

internal fun equals(a: Any?, b: Any?): Boolean = a === b || (a != null && a == b)
 ...
 */
internal fun missingRequiredFields(vararg args: Any?): IllegalStateException {
  var plural = ""
...
internal fun checkElementsNotNull(list: List<*>) {
...
internal fun checkElementsNotNull(map: Map<*, *>) {
...
internal fun countNonNull(a: Any?, b: Any?): Int = (if (a != null) 1 else 0) + (if (b != null) 1 else 0)

/** Returns the number of non-null values in `a, b, c`. */
internal fun countNonNull(a: Any?, b: Any?, c: Any?): Int {
  return (if (a != null) 1 else 0) + (if (b != null) 1 else 0) + (if (c != null) 1 else 0)
}

/** Returns the number of non-null values in `a, b, c, d, rest`. */
internal fun countNonNull(a: Any?, b: Any?, c: Any?, d: Any?, vararg rest: Any?): Int {
  var result = 0
  if (a != null) result++
  if (b != null) result++
  if (c != null) result++
