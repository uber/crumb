...
@CrumbConsumable
public enum Lib1Enum {
  FOO;

  public static TypeAdapter<Lib1Enum> typeAdapter() {
    return new TypeAdapter<Lib1Enum>() {
      @Override
      public void write(JsonWriter out, Lib1Enum value) throws IOException {
        out.value(value.name().toLowerCase());
      }

      @Override
      public Lib1Enum read(JsonReader in) throws IOException {
        return Lib1Enum.valueOf(in.nextString().toUpperCase());
      }
    };
  }
